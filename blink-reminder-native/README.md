...
- **智能防打扰 (输入延迟)**：底层监听系统级空闲状态（如 macOS 的 `CoreGraphics` API）。当到达提醒时间时，如果用户正在活跃打字或操作鼠标（空闲时间 < 1秒），动画会自动推迟，直到用户停下输入超过 1 秒后才会温柔出现，绝不打断工作流。
- **双播机制**：单次提醒内连续播放两次波纹（间隔 0.5 秒），确保提醒效果。
- **动态配置**：支持通过托盘菜单实时开启/关闭提醒，配置修改即时生效，无需重启。
...
- `blink_interval_sec`: 眨眼提醒的间隔时间（秒）。
