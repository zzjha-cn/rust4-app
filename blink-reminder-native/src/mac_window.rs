...
pub fn create_ripple_window(mtm: MainThreadMarker) -> Id<NSWindow> {
    unsafe {
...
pub fn show_ripple(window: &NSWindow, duration_sec: f64, proxy: tao::event_loop::EventLoopProxy<crate::AppEvent>, is_replay: bool) {
    unsafe {
        if !is_replay {
...
pub fn hide_ripple(window: &NSWindow) {
    unsafe {
        if let Some(view) = window.contentView() {
            if let Some(layer) = view.layer() {
